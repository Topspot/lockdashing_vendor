...
namespace Jacopo\Authentication\Controllers;
...
class UserController extends Controller {
    ...
    public function __construct(UserValidator $v, FormHelper $fh, UserProfileValidator $vp, AuthenticateInterface $auth) {
        $this->beforeFilter('user', ['only' => ['getList', 'deleteUser', 'addGroup', 'deleteGroup', 'editPermission', 'destroy']]);
...
//        print_r(Input::get('id'));exit;
        if (App::make('authenticator')->getLoggedUser()->getId() == 1 || App::make('authenticator')->getLoggedUser()->getId() == Input::get('id')) {
            try {
...
//        }elseif(App::make('authenticator')->getLoggedUser()->getId() == Input::get('id')){
//             $user = $this->user_repository->find(Input::get('id'));
//             $presenter = new UserPresenter($user);
//        return View::make('laravel-authentication-acl::admin.user.edit')->with(["user" => $user, "presenter" => $presenter]);
        } else {
...
//           print_r($user->id);exit;
            ...
            $superadmin_group = $this->group_repository->all(["name" => "superadmin"])->first();
            $this->user_repository->addGroup($user->id, $superadmin_group->id);
        } catch (JacopoExceptionsInterface $e) {
            DbHelper::rollback();
    ...
    public function editProfile() {
        if (App::make('authenticator')->getLoggedUser()->getId() == 1 || App::make('authenticator')->getLoggedUser()->getId() == Input::get('user_id')) {
